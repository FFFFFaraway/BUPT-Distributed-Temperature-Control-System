  ...
  <div class="container">
    <b-overlay :show="loading" rounded="sm">
      <b-card title="Center" header-tag="header" bg-variant="light">
        ...
        <b-button :variant="power ? 'outline-danger':'outline-info'" v-if="admin=='true'" @click="flipPower" class="mb-2" size="sm">
          <b-icon icon="power" aria-hidden="true"></b-icon> {{ power ? "Power Off":"Power On" }}
        </b-button>
...
export default {
    ...
    computed:{
      ...mapState('center',['power','state','mode','temp','loading'])
    },
    methods:{
      ...mapActions('center',['getCenter','flipPower','setMode','temp_add']),
      setCold(){
