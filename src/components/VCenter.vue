  ...
  <div class="container">
    <b-overlay :show="loading" rounded="sm">
      <b-card title="Center" header-tag="header" bg-variant="light">
        ...
        <b-button
          :variant="power ? 'outline-danger':'outline-info'"
          v-if="admin=='true'"
          @click="flipPower"
          class="mb-2"
          size="sm"
        ...
        <b-button-toolbar size="sm" v-if="power">
          <b-button-group size="sm" class="mb-1">
            ...
            <b-button
              :variant="state=='Standby'?'outline-secondary':'outline-success'"
              disabled
            >{{ state }}</b-button>
          </b-button-group>
...
export default {
  ...
  methods: {
    ...mapActions("center", ["flipPower", "setMode", "temp_add"]),
    setCold() {
